...
frappe.ui.form.on("Kassa", {
    ...
    mode_of_payment: function(frm) {
        if (frm.doc.mode_of_payment && frm.doc.company) {
            frappe.call({
                method: "logistika.erp_for_logistics.doctype.kassa.kassa.get_cash_account_with_currency",
                args: {
    ...
    update_balance: function(frm) {
        if (frm.doc.cash_account && frm.doc.company) {
            frappe.call({
                method: "logistika.erp_for_logistics.doctype.kassa.kassa.get_account_balance",
                args: {
    ...
    set_mode_of_payment_query: function(frm) {
        frm.set_query("mode_of_payment", function() {
            ...
            if (frm.doc.transaction_type === "Конвертация") {
                return {
                    query: "logistika.erp_for_logistics.doctype.kassa.kassa.conversion_mode_of_payment_query",
                    filters: {
                        company: frm.doc.company
                    }
                };
    ...
    set_mode_of_payment_to_query: function(frm) {
        frm.set_query("mode_of_payment_to", function() {
            ...
            if (frm.doc.transaction_type === "Перемещения") {
                return {
                    query: "logistika.erp_for_logistics.doctype.kassa.kassa.transfer_mode_of_payment_query",
                    filters: {
                        company: frm.doc.company,
                        source_mode_of_payment: frm.doc.mode_of_payment
                    }
    ...
    fetch_exchange_rate: function(frm) {
        ...
        frappe.call({
            method: "logistika.erp_for_logistics.doctype.kassa.kassa.get_exchange_rate",
            args: {
    ...
    party: function(frm) {
        ...
        if (frm.doc.party && frm.doc.party_type) {
            ...
            if (in_list(["Customer", "Supplier"], frm.doc.party_type)) {
                frappe.call({
                    method: "logistika.erp_for_logistics.doctype.kassa.kassa.get_party_currency",
                    args: {
