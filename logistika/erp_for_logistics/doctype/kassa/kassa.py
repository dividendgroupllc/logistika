...
class Kassa(Document):
    ...
    def create_dividend_journal_entry(self):
        """Дивиденд uchun Journal Entry yaratish."""
        dividend_account = get_dividend_account(self.company)
...
def get_expense_accounts(doctype, txt, searchfield, start, page_len, filters):
    """Kompaniyaning barcha leaf (is_group=0) Expense accountlarini olish.

    Logistika'ning kontlar rejasida pokiza'dagi kabi maxsus raqamlangan
    guruh yo'q, shuning uchun har qanday Expense root_type'dagi leaf
    account tanlash uchun mavjud bo'ladi.
    """
...
def validate_expense_account(expense_account, company):
    """Expense account kompaniyaga tegishli va leaf Expense account ekanini tekshirish."""
    account = frappe.db.get_value(
        "Account",
        expense_account,
    ...
    if (
        ...
        frappe.throw(_("Выбранный счет должен быть счетом расходов (Expense) компании {0}").format(company))


def get_dividend_account(company):
    """Дивиденд uchun Equity accountni topish.

    Logistika'ning kontlar rejasida pokiza'dagi kabi raqamlangan alohida
    dividend accountlar yo'q — bitta umumiy "Dividends Paid" accounti bor.
    Shuning uchun account_number o'rniga nomi bo'yicha topiladi.
    """
    account = frappe.db.get_value(
        "Account",
        {
            ...
            "root_type": "Equity",
            "is_group": 0,
            "account_name": ["like", "%Dividend%"],
        },
        "name",
    )

    if not account:
        frappe.throw(_("Счет дивидендов (Equity) не найден для компании {0}").format(company))
